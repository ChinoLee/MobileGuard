...
public class TrafficMonitoringActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private void initDate() {
        long totleflow = mSP.getLong("totleflow", 0);
        long usedflow = mSP.getLong("usedflow", 0);
        if (totleflow > 0 & usedflow >= 0) {
            float scale = usedflow / totleflow;
            if (scale > 0.9) {
        ...
        mTotalTV.setText("本月流量：" + Formatter.formatFileSize(this, totleflow));
    ...
    private void registReceiver() {
        ...
        filter.addAction("android.provider.Telephone.SMS_RECEIVED");
